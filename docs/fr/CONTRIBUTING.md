...
There are two ways you can contribute to this project. Vous pouvez soit apporter un seul pixel au tableau déjà créé, soit contribuer au projet lui-même. Par exemple en ajoutant des tests, en mettant à jour le modèle HTML ou en actualisant la documentation.
...
Contribuer à un projet open source peut être intimidant au début. C'est pourquoi nous avons créé 
...
elle vous guidera à chaque étape, du clonage du projet à la création de votre 'Pull Request'.
...
- Un compte Github
- git installé sur votre ordinateur. [Apprenez comment l'installer](https://help.github.com/en/articles/set-up-git)
- [Node.js](https://nodejs.org) et un gestionnaire de paquet, type [npm](https://npmjs.com)
...
5. Ouvrez votre navigateur à l'adresse http://localhost:8080. Vous devriez voir le même résultat que sur https://open-pixel-art.com avec une grille sur le tableau pour vous aider à placer votre pixel.

---

Il y a également une option pour lancer le projet via Docker. Pour lancer le projet avec Docker, suivez les instructions.
...
Si l'image du docker ne se construit pas, il y a probablement une erreur dans votre code et les tests ne passent pas. Si tout est correct, vous pouvez facilement vous connecter en vous rendant à l'adresse suivante : http://localhost:8080. Vous devriez voir le même contenu que sur https://open-pixel-art.com avec une grille sur le tableau qui vous aide à mieux placer votre pixel.
...
Pour créer un nouveau pixel, cherchez une place entre les nombres pour les coordonnées "x" ou "y" dans le tableau "data" du fichier "pixels.json".
Une fois que vous avez trouvé une position disponible, ajoutez un nouveau pixel.
Ce nouveau pixel doit être un objet ayant les quatre propriétés suivantes :
...
La ligne des coordonnées de votre pixel doit être triée d'abord par la coordonnée y, puis par la coordonnée x. Si vous n'êtes pas sûr de vos modifications, faites le changement et lancez `npm run format:sort-pixels`, il devrait trier votre pixel sur la ligne appropriée.
...
Une fois que vous avez fait votre modifications, allez sur http://localhost:8080 et vous devriez voir votre nouveau pixel. Si vous n'êtes pas satisfait du placement, continuez à changer les valeurs "x" et "y" et si vous n'êtes pas satisfait de la "couleur", vous pouvez continuer à changer la propriété "color".
...
Ensuite, vous devrez choisir votre modification (fichier ayant été modifié) et l'appliquer au futur commit en faisant :
...
Le projet est alimenté par [Eleventy](https://11ty.io). Les fichiers de configuration du projet sont là
...
Le répertoire `_data` contient le fichier `pixels.json` qui représente chaque pixel du tableau, un fichier `defaults.json` qui contient toutes les valeurs par défaut comme la taille du tableau, et le fichier `env.js` pour toutes les variables d'environnement. Toutes les données seront automatiquement disponibles dans le fichier `index.njk`
